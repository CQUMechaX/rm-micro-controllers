...
__ALIGN_BEGIN static uint8_t USBD_CDC_CfgHSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x00,
  0x02 << 1,                                  /* bNumInterfaces: 2 interface (* 2)*/
...
/**
  ...
  * @retval status
  */
uint8_t USBD_CDC_TransmitPacket(USBD_HandleTypeDef *pdev, uint8_t epindex)
{
  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef *)pdev->pClassData;
  ...
  if (hcdc->TxState == 0U)
  {
    ...
    (void)USBD_LL_Transmit(pdev, (epindex < 2) ? CDC_IN_EP : (CDC_IN_EP + 2), hcdc->TxBuffer, hcdc->TxLength);
...
/**
  ...
  * @retval status
  */
uint8_t USBD_CDC_ReceivePacket(USBD_HandleTypeDef *pdev, uint8_t epindex)
{
  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef *)pdev->pClassData;
  ...
  {
    /* Prepare Out endpoint to receive next packet */
    (void)USBD_LL_PrepareReceive(pdev, (epindex < 2) ? CDC_OUT_EP : (CDC_OUT_EP + 2), hcdc->RxBuffer,
                                 CDC_DATA_HS_OUT_PACKET_SIZE);
  }
  else
  {
...
/** @note Based on https://www.jianshu.com/p/1e1b22a48c64
  * 
  */