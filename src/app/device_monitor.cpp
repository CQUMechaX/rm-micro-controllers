...
bool DeviceMonitor::register_and_init(CAN_HandleTypeDef & hcan, JointData & joint)
{
  ...
  this->device_joint_[can_location].push_back(DeviceStatus{10, &joint});
...
{
  return this->device_dict_.insert(std::make_pair(key, value)).second;
}
  ...
  for (auto & list_iter : this->device_joint_) {
    for (DeviceStatus & device : list_iter) {
      ...
      if ((device.offline_time = xTaskGetTickCount() - device.tick) > 30) {
        ...
        taskENTER_CRITICAL();
        memmove(&joint->feedback[1], &joint->feedback[0], sizeof(JointData::CtrlInfo) * 4);
        memmove(&joint->feedback[0], &device.data.joint, sizeof(JointData::CtrlInfo));
        taskEXIT_CRITICAL();
        device.online = true;
      }
